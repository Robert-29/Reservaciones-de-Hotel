...
const Reservar = () => {
  ...
  const handleReservar = async () => {
    // Validaciones
    if (!checkIn || !checkOut) {
      setMensaje("Por favor selecciona las fechas de check-in y check-out");
      return;
    }

    if (checkOut <= checkIn) {
      setMensaje("La fecha de check-out debe ser posterior a la fecha de check-in");
      return;
    }

    if (numeroHuespedes < 1 || numeroHuespedes > datos.capacidad_personas) {
      setMensaje(`El número de huéspedes debe estar entre 1 y ${datos.capacidad_personas}`);
      return;
    }

    if (!usuario) {
      setMensaje("Debes iniciar sesión para hacer una reservación");
      navigate('/iniciarsesion');
      return;
    }
    ...
    try {
      const noches = calculateDays(checkIn, checkOut);
      const precioHabitacion = datos.precio * noches;
      const precioServicios = calcularTotal();
      const precioTotal = precioHabitacion + precioServicios;

      const serviciosSeleccionados = seleccionados.map(id =>
      ...
      if (response.ok) {
        // Redirigir a mis reservaciones
        navigate("/misreservaciones");
      } else {
        setMensaje(data.error || "Error al crear la reservación");
      }
    } catch (err) {
      console.error(err);
    ...
    <>
      ...
      <section className="flex px-16 space-x-10 mb-10" >
        ...
        <div className="w-[55%] p-10 shadow-lg rounded-xl space-y-5 " >
          ...
          {mensaje && <p className="text-red-600 font-inter text-center">{mensaje}</p>}
          <div className='flex flex-col space-y-2' >
            <h3 className='font-inter font-semibold' >Información de Contacto</h3>
            <p> <strong>Nombre:</strong> {usuario?.nombre || 'Cargando...'} </p>
            <p> <strong>Email:</strong> {usuario?.email || 'Cargando...'}</p>
          ...
          <button
            ...
            disabled={cargando}
            className='w-full bg-gray-900 text-white font-inter p-3 rounded-md hover:bg-gray-700 transition disabled:bg-gray-400 disabled:cursor-not-allowed'
          >
            {cargando ? 'Procesando...' : 'Reservar Ahora'}
          </button>
        </div>
      </section>
      <Derechos />
